...
def normalize_shortname(value: str) -> str:
...
def build_synthetic_tactic(phase_name: str) -> dict:
    normalized = normalize_shortname(phase_name)
    synthetic_id = f'x-mitre-tactic--{uuid.uuid5(UUID_NAMESPACE, f"synthetic-tactic:{normalized}")}'
    return {
        'type': 'x-mitre-tactic',
        'id': synthetic_id,
        'name': phase_name.replace('-', ' ').title(),
        'x_mitre_shortname': normalized,
        'description': f'Synthetic SPARTA tactic created from technique kill chain phase "{phase_name}".',
    }


def infer_tactics_from_techniques(techniques_by_id: dict[str, dict]) -> dict[str, dict]:
    inferred = {}
    for technique in techniques_by_id.values():
        for phase in technique.get('kill_chain_phases', []):
            ...
            if normalized not in inferred:
                inferred[normalized] = build_synthetic_tactic(phase_name)
    return {item['id']: item for item in inferred.values()}
...
def parse_bundle(bundle: dict) -> tuple[list[dict], dict[str, dict], dict[str, dict], list[str], list[dict], list[dict]]:
    ...
    if not tactics_by_id:
        tactics_by_id = infer_tactics_from_techniques(techniques_by_id)
        if not matrix_order:
            matrix_order = [
                t.get('x_mitre_shortname', t.get('name', '').lower().replace(' ', '-'))
                for t in sorted(tactics_by_id.values(), key=lambda v: v.get('x_mitre_shortname', ''))
            ]

    if matrix_order:
        phase_shortnames = {
            normalize_shortname(phase.get('phase_name', ''))
            for technique in techniques_by_id.values()
            for phase in technique.get('kill_chain_phases', [])
            if phase.get('phase_name')
        }
        for phase_shortname in sorted(phase_shortnames):
            if phase_shortname and phase_shortname not in matrix_order:
                matrix_order.append(phase_shortname)
